// ============================================================
// Python Lexer for PyDead-BIB
// ============================================================
// Tokenizes Python source code into PyToken stream
...
pub enum PyToken {
    ...
    Match,    // 3.10+
    Case,     // 3.10+
    Print,    // Python 2 compat
    Exec,     // Python 2 compat
...
mod tests {
    ...
    fn test_basic_tokens() {
        ...
        assert!(tokens.iter().any(|t| matches!(t, PyToken::Identifier(s) if s == "x")));
        assert!(tokens.iter().any(|t| matches!(t, PyToken::Assign)));
        assert!(tokens.iter().any(|t| matches!(t, PyToken::IntLiteral(42))));
    }

    #[test]
    fn test_keywords() {
        let mut lexer = PyLexer::new("def if else return");
        let tokens = lexer.tokenize();
        assert!(tokens.contains(&PyToken::Def));
        assert!(tokens.contains(&PyToken::If));
        assert!(tokens.contains(&PyToken::Else));
        assert!(tokens.contains(&PyToken::Return));
    }

    #[test]
    fn test_indentation() {
        let mut lexer = PyLexer::new("if True:\n    x = 1\n");
        let tokens = lexer.tokenize();
        assert!(tokens.contains(&PyToken::Indent));
        assert!(tokens.contains(&PyToken::Dedent));
    }

    #[test]
    fn test_fstring() {
        let mut lexer = PyLexer::new("f\"hello {name}\"");
        let tokens = lexer.tokenize();
        assert!(tokens.iter().any(|t| matches!(t, PyToken::FStringStart(_))));
    }

    #[test]
    fn test_walrus() {
        let mut lexer = PyLexer::new("x := 5");
        let tokens = lexer.tokenize();
        assert!(tokens.contains(&PyToken::ColonAssign));
    }

    #[test]
    fn test_hex_literal() {
        let mut lexer = PyLexer::new("0xFF");
        let tokens = lexer.tokenize();
        assert!(tokens.iter().any(|t| matches!(t, PyToken::IntLiteral(255))));
    }

    #[test]
    fn test_binary_literal() {
        let mut lexer = PyLexer::new("0b1010");
        let tokens = lexer.tokenize();
        assert!(tokens.iter().any(|t| matches!(t, PyToken::IntLiteral(10))));
    }
}
